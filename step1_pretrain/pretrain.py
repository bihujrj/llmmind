...
from utils.train_tools import get_lr, is_main_process, init_distributed_mode, lm_checkpoint, setup_seed, init_model, \
    SkipBatchSampler, read_wandb_config
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            loss = res.loss + res.aux_loss  #主损失+moe损失
            loss = loss / args.accumulation_steps #平均损失

        scaler.scale(loss).backward() #损失回传

        # 梯度累积达到指定步数时，执行参数更新
        if (step + 1) % args.accumulation_steps == 0:
            # 将优化器中累积的梯度从缩放状态还原为原始尺度，以便正确地进行梯度裁剪
            scaler.unscale_(optimizer)

            # 对所有模型参数的梯度进行L2范数裁剪，限制其最大范数不超过args.grad_clip，防止梯度爆炸
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)

            # 执行优化器参数更新（如果梯度中有inf/NaN，则自动跳过更新）
            scaler.step(optimizer)

            # 更新缩放因子，根据本次迭代是否出现inf/NaN来动态调整缩放系数
            scaler.update()

            # 将模型参数的梯度置为None，清空累积的梯度，为下一组累积步做准备
            # set_to_none=True 比传统zero_grad()更省内存，并可能提升性能
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            if res.aux_loss is not None:
                if hasattr(res.aux_loss,'item'):
                    current_aux_loss=res.aux_loss.item()
                else:
                    current_aux_loss=float(res.aux_loss)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../../../llm_data/pretrain_hq.jsonl", help="预训练数据路径")
    parser.add_argument("--tokenizer_path", type=str, default="../model_ref", help="预训练数据路径")
    ...
    lm_config = LlmConfig(hidden_size=args.hidden_size, num_deep_layers=args.num_hidden_layers,use_moe=bool(args.use_moe))
    ...
    if args.use_wandb and is_main_process():
        ...
        if args.wandb_key is None:
            wandb_key=read_wandb_config()
            wandb.login(key=args.wandb_key)
            args.wandb_key=wandb_key
        wandb.init(project="llmmind", config=vars(args))
    ...
    scaler = torch.cuda.amp.GradScaler(enabled=(args.dtype == 'float16'))#自动混合精度
